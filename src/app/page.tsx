...
export default function Home() { return <><section className="hero"><img src={assetUrl("/assets/home/hero.webp")} alt="Ralph Studio 影像创作现场" fetchPriority="high" decoding="sync" /><div className="hero-shade" /><div className="hero-content"><p className="eyebrow">Visual Creator &amp; AI Workflow Builder</p><h1>Ralph <em>Studio</em></h1><p className="hero-skills">影像创作 · 后期制作 · AI 工作流搭建</p><p className="hero-copy">从影像创作到自动化流程设计，我关注如何把创意、执行与工具链连接起来，让内容生产更高效、更清晰、更可复用。</p><div className="button-row"><Link className="button button--primary" href="/works/">查看作品</Link><Link className="button" href="/cases/">AI 工作流案例</Link><Link className="button button--quiet" href="/contact/">联系我</Link></div></div><div className="scroll-cue">SCROLL <span /></div></section>
  <section className="section"><RevealOnScroll><SectionHeading eyebrow="SELECTED WORK" title="精选作品" text="聚焦影像制作、后期表达和现场执行的代表性实践。" /><div className="works-grid works-grid--featured">{works.filter((work) => work.featured).slice(0, 5).map((work, index) => <WorkCard key={work.id} work={work} index={index} priority={index < 2} />)}</div><Link className="section-link" href="/works/">浏览全部作品 <b>↗</b></Link></RevealOnScroll></section>
  ...
  <section className="section"><RevealOnScroll><SectionHeading eyebrow="AI WORKFLOW" title="从工具到内容生产流程" text="不把 AI 视为孤立能力，而是作为内容生产环节中的辅助与连接。" /><div className="case-grid">{cases.filter((item) => item.category === "AI 工作流").slice(0, 2).map((item, index) => <CaseCard key={item.id} item={item} index={index} priority />)}</div><Link className="section-link" href="/cases/">查看全部案例 <b>↗</b></Link></RevealOnScroll></section>
